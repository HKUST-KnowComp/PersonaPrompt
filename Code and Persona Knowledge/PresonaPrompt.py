...
from PresonaVerbalizer import PresonaVerbalizer
from data_utils import KialoProcessor, DDOProcessor, PersonaKnowledge_DDOProcessor, PersonaKnowledgeProcessor, PersonaKnowledge_ablationstudy
...
parser.add_argument("--project_root", default='./PromptArgument', help="The project root in the file system")
parser.add_argument("--template_file", default="./PromptArgument/Argument_template.txt", help="The project root in the file system")                    
parser.add_argument("--template_id", type=int, default=0)
parser.add_argument("--verbalizer_id", type=int, default=0)

parser.add_argument("--setting",type=str, default= "argu" )
# parser.add_argument("--dataset",type=str, default= "ji" )
parser.add_argument("--result_file", type=str, default="./PromptArgument/result/results.txt")
parser.add_argument("--ckpt_file", type=str, default="ArgumentImpactClassification_result")
...
if args.setting == "argu":
    data_dir_train = "./PromptArgument/datas"
    data_dir_dev = "./PromptArgument/datas"
    data_dir_test = "./PromptArgument/datas"
    ...
    label_words = {
    "IMPACTFUL"     :["for example", "Impactful"],
    "MEDIUM IMPACT" :["also", "Medium Impactful"],
    "NOT IMPACTFUL" :["instead", "Not Impactful"],
    }

elif args.setting == "argu-PersonaKnowledge":
    data_dir_train = "./PromptArgument/datas"
    data_dir_dev = "./PromptArgument/datas"
    data_dir_test = "./PromptArgument/datas"
    Processor = PersonaKnowledgeProcessor()
    ...
    label_words = {
    "IMPACTFUL"     :["Impactful"],
    "MEDIUM IMPACT" :["Medium Impactful"],
    "NOT IMPACTFUL" :["Not Impactful"],
    }
    
elif args.setting == "argu-PersonaKnowledge4dimension_ablationstudy":
    data_dir_train = "./PromptArgument/datas"
    data_dir_dev = "./PromptArgument/datas"
    data_dir_test = "./PromptArgument/datas"
    Processor = PersonaKnowledge_ablationstudy()
...
mytemplate = PTRTemplate(model=plm, tokenizer=tokenizer).from_file("./Argument_template.txt", choice=template_id)
myverbalizer = PresonaVerbalizer(tokenizer, classes=class_labels, num_classes=3, label_words = label_words)
...
for epoch in range(1000000):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        if actual_step % gradient_accumulation_steps == 0 and glb_step > 0 and glb_step % eval_every_steps == 0:
            ...
            print(
            "Glb_step {}, val_f1score {}, average time {}".format(glb_step, val_acc, tot_train_time / actual_step), flush
            =True)
            prompt_model.train()
